...
export const actions = {
    searchDocList: (department_id, stage=0, items_per_page=0, page_id=0) => {
      console.log(`searchDocList: dep_id: ${department_id}`);
      return async (dispatch, getState) => {
        try {
          if (shouldFetchGroups(stage, getState())) {
            dispatch(appActions.startRequest());
            const data = await progressdocApi.queryDocListByKeyword(department_id, stage, items_per_page, page_id);
